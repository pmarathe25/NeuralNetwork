...
namespace ai {
    ...
    template <typename T>
    __global__ void computeFeedForwardNoIntermediate(const T* input, const T* weight, const T* bias, int numRowsActivatedOutput,
        int numColsActivatedOutput, int numColsWeight, int inputSize, int weightSize, T* activatedOutput, int aFunc) {
        __shared__ T tileActivated[BLOCK_DIM][BLOCK_DIM + 1];
        __shared__ T tileWeight[BLOCK_DIM][BLOCK_DIM + 1];
        // Compute the coordinates of matrix C that this thread is responsible for.
        int row = blockIdx.x * blockDim.x + threadIdx.x;
