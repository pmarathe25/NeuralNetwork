...
namespace ai {
    ...
    template <typename T>
    __global__ void computeFeedForward(const T* activationOutput, const T* weight, const T* bias, int numRowsActivatedOutput,
        int numColsActivatedOutput, int numColsWeight, int activationOutputSize, int weightSize, T* output, T* activationOutputNext, int aFunc) {
       ...
       for (int i = 0; i < maxIterations; i += BLOCK_DIM) {
           ...
           // Load sub-matrix A.
           tileActivated[threadIdx.x][threadIdx.y] = (indexActivatedOutput < activationOutputSize) ? activationOutput[indexActivatedOutput] : 0;
           // Load sub-matrix B.
       ...
       if (cValid) {
           ...
           switch (aFunc) {
               case 0:
                   activationOutputNext[row * numColsWeight + col] = 1 / (1 + exp(-outValue));
                   break;
               case 1:
                   activationOutputNext[row * numColsWeight + col] = log(1 + exp(outValue));
                   break;
           }
       }
    }
